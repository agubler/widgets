...
function getFindKey(findOptions: ResourceFindOptions<any>) {
	...
	return `size-${options.size}-query-${JSON.stringify(options.query)}-find-${JSON.stringify(
		find
	)}`;
}
...
function createOptionsWrapper(): OptionsWrapper<any> {
	...
	return {
		options(
			invalidator: Invalidator,
			newOptions?: Partial<ResourceOptions<any>>
		): ResourceOptions<any> {
...
function createResource<S = never>(template: ResourceTemplate<S>, data?: S[]): Resource<S> {
	...
	function put(
		response: ResourceFindResponse<S> | undefined,
		request: ResourceFindRequest<S>
	): void;
	function put(
...
const memoryTemplate: ResourceTemplate<any> = {
	read: (request, { get, put }) => {
		const { data } = get();
		const filteredData = request.query
...
export function createResourceTemplate<S = void>(
	...
	const resourceTemplateFactory: any = (
		options: { transform?: TransformConfig<any, S>; data?: S[] } = {}
	) => {
...
export function createResourceMiddleware<T = never>() {
	const factory = create({ invalidator, destroy, diffProperty }).properties<
		ResourceMiddlewareProperties<T>
	>();
	...
	return factory(({ middleware: { invalidator, destroy, diffProperty }, properties }) => {
		...
		const customResourceWrapperMap = new Map<
			string,
			{ wrapper: ResourceWrapper<T, T>; data?: T[] }
		>();
		destroy(() => {
		...
		diffProperty(
			'resource',
			properties,
			({ resource: currentResource }, { resource: nextResource }) => {
				if (isTemplate(nextResource)) {
					const current = isTemplate(currentResource)
						? currentResource
						: ({} as ResourceWithTemplate<T>);
					...
					if (
						(isWrapper(currentResource) &&
							currentResource.resource !== nextResource.resource) ||
						!isWrapper(currentResource)
					) {
						invalidator();
					}
		...
		return (
			resourceMiddlewareOptions: ResourceMiddlewareOptions<T> = {}
		): ResourceMiddleware<T> => {
			...
			function getOrRead(options: ResourceOptions<T>): (undefined | T[])[] {
				...
				if (data && transform) {
					return data.map((items: any) =>
						items.map((item: any) => transformData(item, transform))
					);
				}
				return data;
			}
