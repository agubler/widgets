...
export const List = factory(function List({
	...
	function renderItems(start: number, count: number, startNode: number) {
		...
		if (metaInfo && metaInfo.total) {
			...
			for (let i = 0; i < Math.min(metaInfo.total - start, count); i++) {
				const index = i + startNode;
				const pageIndex = pages.indexOf(Math.floor(index / count) + 1);
				const indexWithinPage = index - Math.floor(index / count) * count;
	...
	if (!options().page || !options().page) {
		const { size = renderedItemsCount, page = 1, query } = options();
		options({
			page,
			size,
			query
